...
1. **🚀 代理节点选择**（基础节点池，包含所有可用节点）
2. **🏠 家宽节点选择**（手动指定用于对原生 IP 要求极高的节点）
3. **🎵 TikTok**（默认：`🏠 家宽节点选择`，备用：代理选择/直连）
4. **🤖 AI 服务**（默认：`🏠 家宽节点选择`，备用：代理选择/直连）
5. **🔍 Google**（默认：`🏠 家宽节点选择`，备用：代理选择/直连）
6. **Ⓜ️ 微软服务**（默认：`DIRECT`，备用：代理选择/家宽选择）
7. **🇺🇳 国外网站**（默认：`🚀 代理节点选择`，备用：家宽选择/直连）
8. **🇨🇳 国内网站**（默认：`DIRECT`，备用：代理选择/家宽选择）
9. **🐟 规则之外**（默认：`DIRECT`，备用：代理选择/家宽选择）
